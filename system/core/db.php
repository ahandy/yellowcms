...
class Db extends DbBlueprint {
	...
	**/
	protected function connectToDB() {
		try {
   			$dbh = new PDO(get_setting('database_type') . ':host=' . get_setting('database_host') . ';dbname=' . get_setting('database_name'), get_setting('database_user'), get_setting('database_pass'));
	...
	**/
	...
	public function returnTables() {
		$q = $this -> dbh -> query("show tables WHERE `Tables_in_cms` NOT IN ('tables', 'field_types', 'users')");
